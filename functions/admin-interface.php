...
function woothemes_options_page(){
	...
	$update_message = '';
    if(get_option('woo_theme_version_checker') == 'true') {
...
function woo_load_only() {
	...
	function woo_admin_head() { 
		...
		jQuery(document).ready(function(){
			...
			//JQUERY DATEPICKER
			if (jQuery('#woo_calendar').length) {
				jQuery('#woo_calendar').datepicker();
			}
...
function woothemes_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'calendar':
			...
			$std = get_option($value['id']);
			if ( $std != "") { $val = $std; }
            $output .= '<input class="woo-input-text" type="text" name="'.$value['id'].'" id="'.$value['id'].'" value="'.$val.'">';
...
function woothemes_version_checker ($local_version) {
	...
	$rss = fetch_feed('http://www.woothemes.com/?feed=updates&theme=' . get_option('template'));
