			...
			if ($data_insert) {
				...
				foreach ($pages_array as $post) 
				{
					...
					if ($post->post_parent == 0) 
					{
						...
						//INSERT page								
						$insert = "INSERT INTO ".$table_name." (position,post_id,parent_id,custom_title,custom_link,custom_description,menu_icon,link_type,menu_id,custom_anchor_title) "."VALUES ('".$counter."','".$post->ID."','".$parent_id."','".$post->post_title."','".get_permalink($post->ID)."','','','page','1','".$post->post_title."')";
	  					$results = $wpdb->query( $insert );
	  					$counter++;
...
function woo_custom_nav_scripts() {
	...
	//JQUERY Dialog box
	echo '<script src="'.get_bloginfo('url').'/wp-includes/js/jquery/ui.dialog.js" type="text/javascript"></script>';
			...
			<div class="widgets-holder-wrap">
				...
				<div class="widget-holder">
					...
					if ($pages_array)
					{
						...
						{
							//Add page name to 
							$page_name .= htmlentities($post->post_title, ENT_QUOTES, 'UTF-8').'|';
						}
					}
					else
					{
					...
					if ($categories)
					{
						...
						{ 
							...
							if (isset($category_names[0]->name))
							{
								...
								$cat_name .= htmlentities($category_names[0]->name, ENT_QUOTES, 'UTF-8').'|';
							}
				  		}
				  	}
				  	else
		...
		foreach ($woo_custom_nav_menu as $woo_custom_nav_menu_items) {
			...
			if ($woo_custom_nav_menu_items->link_type == 'page')
			{
				...
				if ($woo_custom_nav_menu_items->custom_title == '') {
					$title = htmlentities(get_the_title($woo_custom_nav_menu_items->post_id), ENT_QUOTES, 'UTF-8');
				}
				else {
					$title = htmlentities($woo_custom_nav_menu_items->custom_title, ENT_QUOTES, 'UTF-8');
				}

				if ($woo_custom_nav_menu_items->custom_description == '') {
					$description = htmlentities(get_post_meta($woo_custom_nav_menu_items->post_id, 'page-description', true), ENT_QUOTES, 'UTF-8');
				}
				else {
					$description = htmlentities($woo_custom_nav_menu_items->custom_description, ENT_QUOTES, 'UTF-8');
				}
				$target = '';
			}
			//Category Menu Item
			elseif ($woo_custom_nav_menu_items->link_type == 'category') 
			{
				...
				if ($woo_custom_nav_menu_items->custom_title == '') {
					...
					$title =  htmlentities($title_raw[0]->cat_name, ENT_QUOTES, 'UTF-8');	
				}
				else {
					$title = htmlentities($woo_custom_nav_menu_items->custom_title, ENT_QUOTES, 'UTF-8');
				}
				...
				if ($woo_custom_nav_menu_items->custom_description == '') {
					$description = htmlentities(strip_tags(category_description($woo_custom_nav_menu_items->post_id)), ENT_QUOTES, 'UTF-8');
				}
				else {
					$description = htmlentities($woo_custom_nav_menu_items->custom_description, ENT_QUOTES, 'UTF-8');
				}
			...
			if (isset($woo_custom_nav_menu_items->custom_anchor_title)) {
				$anchor_title = htmlentities($woo_custom_nav_menu_items->custom_anchor_title, ENT_QUOTES, 'UTF-8');
			}
			else {
				$anchor_title = $title;
			}
			...
			if (($queried_id == $woo_custom_nav_menu_items->post_id) && ($queried_id != 0) ) {
				$li_class = 'class="current_page_item"';
			}
			else if (($woo_custom_nav_menu_items->custom_link == $full_web_address) && ($queried_id == 0) ) {
				$li_class = 'class="current_page_item"';
			}
			else {
				$li_class = '';
			...
			?><?php 
					...
					if ($type == "frontend")
					{	        
						?><li <?php echo $li_class; ?>><a title="<?php echo $anchor_title; ?>" href="<?php echo $link; ?>" <?php echo $target; ?>><?php echo $before_title.$title.$after_title; ?><?php 
					...
					if ($woo_custom_nav_menu_items->parent_id == 0) 
					{
						...
						{
							//Recursive function
							$intj = woo_custom_navigation_sub_items($woo_custom_nav_menu_items->id,$woo_custom_nav_menu_items->link_type,$table_name,$type,$woo_custom_nav_menu_id);
						}
						//BACKEND
						else 
						{
		...
		foreach ($woo_custom_nav_menu as $sub_item) 
		{
			...
			if ($sub_item->link_type == 'category') 
			{
				...
				if ($sub_item->custom_title == '') {
					...
					$title = htmlentities($sub_item->custom_title, ENT_QUOTES, 'UTF-8');
				}
			...
			elseif ($sub_item->link_type == 'page')
			{
				...
				if ($sub_item->custom_title == '') {
					$title = htmlentities(get_the_title($sub_item->post_id), ENT_QUOTES, 'UTF-8');
				}
				else {
					$title = htmlentities($sub_item->custom_title, ENT_QUOTES, 'UTF-8');
				}
			...
			if (($queried_id == $sub_item->post_id) && ($queried_id != 0)) {
				$li_class = 'class="current_page_item"';
			}
			else if (($sub_item->custom_link == $full_web_address) && ($queried_id == 0) ) {
				$li_class = 'class="current_page_item"';
			}
			else {
				$li_class = '';
			...
			if (isset($sub_item->custom_anchor_title)) {
				$anchor_title = htmlentities($sub_item->custom_anchor_title, ENT_QUOTES, 'UTF-8');
			}
			else {
				$anchor_title = $title;
			}
			...
			if ($post->post_parent == 0)
			{
				...
				if ($type == 'menu')
				{
					...
					<li id="menu-<?php echo $intCounter; ?>" value="<?php echo $intCounter; ?>">
						...
						<input type="hidden" name="title<?php echo $intCounter; ?>" id="title<?php echo $intCounter; ?>" value="<?php echo htmlentities($post->post_title, ENT_QUOTES, 'UTF-8'); ?>" />
						...
						<input type="hidden" name="anchortitle<?php echo $intCounter; ?>" id="anchortitle<?php echo $intCounter; ?>" value="<?php echo htmlentities($post->post_title, ENT_QUOTES, 'UTF-8'); ?>" />
				...
				elseif ($type == 'default')
				{
					?>
					 ...
					 <li>
				        ...
				        <dt>
				        <?php
				        	$post_text = htmlentities($post->post_title, ENT_QUOTES, 'UTF-8');
				        	$post_url = get_permalink($post->ID);
				        	$post_id = $post->ID;
				        	$post_parent_id = $post->post_parent;
				        	
							$description = htmlentities(get_post_meta($post_id, 'page-description', true), ENT_QUOTES, 'UTF-8');
			...
			if ($cat_item->parent == 0)
			{
				...
				if ($type == 'menu')
				{
			    	...
			    	<li id="menu-<?php echo $intCounter; ?>" value="<?php echo $intCounter; ?>">
			            ...
			            <input type="hidden" name="parent<?php echo $intCounter; ?>" id="parent<?php echo $intCounter; ?>" value="0" />
			            <input type="hidden" name="title<?php echo $intCounter; ?>" id="title<?php echo $intCounter; ?>" value="<?php echo htmlentities($cat_item->cat_name, ENT_QUOTES, 'UTF-8'); ?>" />
						...
						<input type="hidden" name="description<?php echo $intCounter; ?>" id="description<?php echo $intCounter; ?>" value="<?php echo htmlentities($cat_item->category_description, ENT_QUOTES, 'UTF-8'); ?>" />
						<input type="hidden" name="icon<?php echo $intCounter; ?>" id="icon<?php echo $intCounter; ?>" value="0" />
						<input type="hidden" name="position<?php echo $intCounter; ?>" id="position<?php echo $intCounter; ?>" value="<?php echo $intCounter; ?>" />
						<input type="hidden" name="linktype<?php echo $intCounter; ?>" id="linktype<?php echo $intCounter; ?>" value="category" />
						<input type="hidden" name="anchortitle<?php echo $intCounter; ?>" id="anchortitle<?php echo $intCounter; ?>" value="<?php echo htmlentities($cat_item->cat_name, ENT_QUOTES, 'UTF-8'); ?>" />
						<input type="hidden" name="newwindow<?php echo $intCounter; ?>" id="newwindow<?php echo $intCounter; ?>" value="0" />
			    ...
			    elseif ($type == 'default')
			    {
			    	...
			    	<li>
						...
						<dt>
						<?php
	        			$post_text = htmlentities($cat_item->cat_name, ENT_QUOTES, 'UTF-8');
	        			$post_url = get_category_link($cat_item->cat_ID);
	        			$post_id = $cat_item->cat_ID;
	        			$post_parent_id = $cat_item->parent;
	        			$description = htmlentities(strip_tags($cat_item->description), ENT_QUOTES, 'UTF-8');
	        			?>
	        			<?php $templatedir = get_bloginfo('template_directory'); ?>
	...
	if ($sub_array)
	{
		...
		foreach ($sub_array as $sub_item)
		{
			...
			if ($type == 'categories') 
			{
				...
				$title = htmlentities($sub_item->cat_name, ENT_QUOTES, 'UTF-8');
				$parent_id = $sub_item->cat_ID;
				$itemid = $sub_item->cat_ID;
				$linktype = 'category';
				$appendtype = 'Category';
				$description = htmlentities(strip_tags($sub_item->description), ENT_QUOTES, 'UTF-8');				
			}
			//Page Menu Item
			elseif ($type == 'pages')
			{
				$link = get_permalink($sub_item->ID);
				$title = htmlentities($sub_item->post_title, ENT_QUOTES, 'UTF-8');
				$parent_id = $sub_item->ID;
				$linktype = 'page';
				$itemid = $sub_item->ID;
				$appendtype = 'Page';
				$description = htmlentities(get_post_meta($itemid, 'page-description', true), ENT_QUOTES, 'UTF-8');
			}
			//Custom Menu Item
			else 
			{
