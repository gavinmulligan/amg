    ...
    if(        
		...
		OR      $woo_metabox['type'] == 'images') {
            ...
            elseif ($woo_metabox['type'] == 'calendar'){
                ...
                $output .= "\t".'<tr>';
                $output .= "\t\t".'<th class="woo_metabox_names"><label for="'.$woo_metabox.'">'.$woo_metabox['label'].'</label></th>'."\n";
                $output .= "\t\t".'<td><input class="woo_input_text" type="text" name="woothemes_'.$woo_metabox["name"].'" id="'.$woo_id.'" value="'.$woo_metaboxvalue.'">';
                $output .= '<span class="woo_metabox_desc">'.$woo_metabox['desc'].'</span></td>'."\n";
                $output .= "\t".'<td></td></tr>'."\n";  
            ...
            $output .= "\t\t".'<td>';
                ...
                foreach ( $array as $id => $option ) {
                    ...
                    if($woo_metaboxvalue == $option) { $checked = ' checked';} else {$checked='';}
                        ...
                        $output .= '<input type="radio" '.$checked.' value="' . $id . '" class="woo_input_radio"  name="woothemes_'. $woo_metabox["name"] .'" />';
			...
			foreach ($woo_metabox['options'] as $key => $option) 
				 ...
				 if($woo_metaboxvalue != '') {
					...
					$layout .= '<input type="radio" id="woo-meta-radio-img-' . $woo_metabox["name"] . $i . '" class="checkbox woo-meta-radio-img-radio" value="'.$key.'" name="woothemes_'. $woo_metabox["name"].'" '.$checked.' />';
		...
		break;        
            ...
            elseif($woo_metabox['type'] == 'upload')
            {
                ...
                $output .= "\t".'<tr>';
                $output .= "\t\t".'<th class="woo_metabox_names"><label for="'.$woo_id.'">'.$woo_metabox['label'].'</label></th>'."\n";
                $output .= "\t\t".'<td class="woo_metabox_fields">'. woothemes_uploader_custom_fields($post->ID,$woo_metabox["name"],$default,$woo_metabox["desc"]);
                $output .= '</td>'."\n";
                $output .= "\t".'</tr>'."\n";
...
function woothemes_metabox_handle(){   
    ...
    if (isset($_POST['action']) && $_POST['action'] == 'editpost'){                                   
        foreach ($woo_metaboxes as $woo_metabox) { // On Save.. this gets looped in the header response and saves the values submitted
            if($woo_metabox['type'] == 'text' OR $woo_metabox['type'] == 'calendar' OR $woo_metabox['type'] == 'select' OR $woo_metabox['type'] == 'checkbox' OR $woo_metabox['type'] == 'textarea' OR $woo_metabox['type'] == 'images' ) // Normal Type Things...
                {
